...
onMounted(async () => {
  try {
    ...
    if (!isSuccess && txnRef) {
      console.log("Thanh toán thất bại, huỷ booking...");
      await api.post(`/bookings/cancel-by-txn/${txnRef}`);
      console.log("Đã huỷ các ghế giữ chỗ cho giao dịch:", txnRef);
      return;
    }

    // Nếu thanh toán thành công
    if (isSuccess) {
      console.log("Thanh toán thành công. Đang xác nhận vé...");

      const res = await api.post(`/bookings/confirm-payment/${txnRef}`);
      console.log("Kết quả confirm-payment:", res.data);
